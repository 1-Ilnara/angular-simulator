...
export class HomePageComponent {
  ...
  onSearch(): void {
    if (this.isSearchInvalid) {
      this.messageService.showError('Пожалуйста, заполните все поля поиска!');
      return;
    }
  ...
  onLoadMoreBlogs(): void {
    this.messageService.showWarn('Загрузка дополнительных материалов...');
  }

  onViewTourProgram(event: Event): void {
    event.preventDefault();
    this.messageService.showInfo('Программа тура открыта для просмотра.');
  }

  onViewProgramsPrice(): void {
    this.messageService.showSuccess('Информация о стоимости программ загружена.');
  }

  onViewDestinationsRating(): void {
    this.messageService.showInfo('Рейтинг направлений успешно обновлен!');
  }
